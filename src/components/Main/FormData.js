...
export default function FormData({ bikes, onCreate }) {
	...
	function submitHandler(event) {
		...
		if (Number.isNaN(price) || Number.isNaN(id) || Number.isNaN(wheelSize)) {
			setErrors({
      ...
      || description.length < 5
			...
			setErrors({
				message:
          "Минимальная длина всех текстовых полей должна составлять 5 символов",
			});
			setTimeout(() => {
				setErrors("");
			}, 3000);
	...
	return (
		<div>
			<form onSubmit={submitHandler} className={"bike-form"}>
				{errors ? (
					<h1 className={"error-message"}>{errors.message}</h1>
				) : (
					<h1> </h1>
				)}
